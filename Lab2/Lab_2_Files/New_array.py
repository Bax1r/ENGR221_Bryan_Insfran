...
class NewArray():
    ...
    def __init__(self, initialSizeOrValues):
        if type(initialSizeOrValues) == int: # Checks if integer value
            self.__a = [None] * initialSizeOrValues
            self.__length = initialSizeOrValues
        else:
            self.__a = initialSizeOrValues # The array stored as a list
            self.__length = len(initialSizeOrValues)# Start with no values in the list
